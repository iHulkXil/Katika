...
const games: Game[] = [
  { name: 'Dice', description: 'A clean, classic roll with a modern edge.', icon: <Dice5 />, accent: 'from-emerald-400/20 to-emerald-950/10', index: '01' },
  ...
  return <div className="min-h-[100dvh] bg-background">
    <header className="border-b border-border/70 bg-background/90 backdrop-blur-md sticky top-0 z-40">
      <div className="mx-auto flex h-[68px] max-w-[1240px] items-center justify-between px-5 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/wallet" data-testid="link-connect-header" className="hidden rounded-lg bg-secondary px-3.5 py-2 text-xs font-semibold text-secondary-foreground transition-transform hover:-translate-y-0.5 sm:block">Connect wallet</Link>
        </div>
      </div>
    </header>
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-border/80 bg-card/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl md:hidden" aria-label="Mobile navigation">
      <div className="mx-auto flex max-w-md items-center justify-around">
        {navItems.map(({ href, label, icon: Icon }) => <Link key={href} href={href} data-testid={`link-mobile-${label.toLowerCase()}`} className={`flex min-w-[52px] flex-col items-center gap-1 rounded-lg py-1 text-[10px] ${location === href ? 'text-primary' : 'text-muted-foreground'}`}><Icon size={19} /><span>{label}</span></Link>)}
      </div>
    </nav>
  </div>;
}
...
function GameCard({ game, featured = false }: { game: Game; featured?: boolean }) {
  return <div data-testid={`card-game-${game.name.toLowerCase().replace(' ', '-')}`} className={`group relative overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-br ${game.accent} ${featured ? 'min-h-[290px] md:min-h-[330px]' : 'min-h-[240px]'} p-5 transition-all hover:-translate-y-1 hover:border-primary/50`}>
    ...
    <div className="relative flex h-full flex-col">
      <div className="flex items-start justify-between"><span className="font-mono-custom text-[11px] tracking-widest text-muted-foreground">{game.index} / FUTURE</span><span className="rounded-full border border-secondary/30 px-2 py-1 font-mono-custom text-[10px] text-secondary">COMING SOON</span></div>
...
function Home() {
  return <div className="overflow-hidden">
    <header className="absolute left-0 right-0 top-0 z-20"><div className="mx-auto flex h-[76px] max-w-[1240px] items-center justify-between px-5 lg:px-8"><Brand light /><div className="flex items-center gap-3"><Link href="/games" className="hidden text-sm text-muted-foreground transition-colors hover:text-foreground sm:block" data-testid="link-explore-games">Explore games</Link><ButtonLink href="/wallet" variant="quiet" testId="link-connect-wallet-top">Connect wallet <ArrowRight size={14} /></ButtonLink></div></div></header>
    <section className="relative min-h-[680px] border-b border-border/70 surface-grid">
      ...
      <div className="relative mx-auto flex min-h-[680px] max-w-[1240px] items-center px-5 pb-12 pt-28 lg:px-8"><div className="max-w-[720px] reveal">
        ...
        <div className="mt-9 flex flex-wrap gap-3"><ButtonLink href="/games" testId="button-play-demo">Play Demo <ArrowRight size={16} /></ButtonLink><ButtonLink href="/wallet" variant="quiet" testId="button-connect-wallet">Connect Wallet <WalletCards size={16} /></ButtonLink></div>
    ...
    <section className="mx-auto max-w-[1240px] px-5 py-20 lg:px-8"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="font-mono-custom text-[11px] tracking-[.22em] text-secondary">01 / THE LOBBY</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.06em] sm:text-5xl">Meet the first four.</h2></div><p className="max-w-[320px] text-sm leading-6 text-muted-foreground">A considered collection of familiar games. The tables are quiet for now — they are being prepared.</p></div><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{games.map((game, i) => <GameCard key={game.name} game={game} featured={i === 0} />)}</div></section>
    <section className="border-y border-border/70 bg-card/30"><div className="mx-auto grid max-w-[1240px] gap-10 px-5 py-20 lg:grid-cols-[.9fr_1.4fr] lg:px-8"><div><p className="font-mono-custom text-[11px] tracking-[.22em] text-primary">02 / OUR APPROACH</p><h2 className="mt-4 max-w-md text-4xl font-semibold leading-tight tracking-[-.06em]">Trust is part of the interface.</h2><p className="mt-5 max-w-md text-sm leading-6 text-muted-foreground">We are building a place where every detail earns its place. No noise, no pressure — just a clear foundation for what comes next.</p></div><div className="grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-3">{[['01','Wallet-based access','Your connection is your key.'],['02','Fair gameplay','Clarity comes before complexity.'],['03','Demo mode','A safe space to explore while we build.']].map(([num,title,copy]) => <div key={num} className="bg-background p-6"><span className="font-mono-custom text-xs text-secondary">{num}</span><h3 className="mt-14 font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{copy}</p></div>)}</div></div></section>
...
function Dashboard() {
  return <><PageIntro eyebrow="01 / OVERVIEW" title="Welcome to Katika.Bet" description="Your foundation for the next generation of play. Everything here is visual-only while the platform is being prepared." /><div className="mx-auto max-w-[1240px] px-5 py-10 lg:px-8"><div className="grid gap-4 md:grid-cols-[1.35fr_1fr_1fr]"><StatCard label="Demo Balance" value="Coming Soon" icon={<Coins />} /><StatCard label="Leaderboard" value="Coming Soon" icon={<Trophy />} /><StatCard label="Rewards" value="Coming Soon" icon={<Crown />} /></div><div className="mt-12 flex items-center justify-between"><div><p className="font-mono-custom text-[11px] tracking-[.22em] text-secondary">AVAILABLE SOON</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">Games</h2></div><Link href="/games" className="flex items-center gap-1 text-sm text-primary hover:underline" data-testid="link-dashboard-games">View lobby <ChevronRight size={15} /></Link></div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{games.map(game => <GameCard key={game.name} game={game} />)}</div></div></>;
}
...
function Games() {
  return <><PageIntro eyebrow="02 / GAME LOBBY" title="A table for every kind of curious." description="These are the first games planned for Katika.Bet. Select a card to see its direction — play functionality arrives in a future sprint." /><div className="mx-auto max-w-[1240px] px-5 py-10 lg:px-8"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{games.map(game => <GameCard key={game.name} game={game} />)}</div><div className="mt-10 rounded-2xl border border-dashed border-border bg-card/40 p-6 text-center"><Sparkles size={20} className="mx-auto text-secondary" /><p className="mt-3 text-sm text-foreground">More tables are being considered.</p><p className="mt-1 text-xs text-muted-foreground">No game logic, wagering, or demo credits are active in Sprint 1.</p></div></div></>;
}
...
function Profile() { return <Placeholder kind="06 / PROFILE" title="Your profile, when you're ready." copy="Profile preferences and wallet identity will have a home here." icon={UserRound} />; }
function Wallet() { return <Placeholder kind="03 / WALLET" title="Your wallet, on your terms." copy="Wallet-based access is part of the vision. Connection and account functionality arrives in a future sprint." icon={WalletCards} />; }
