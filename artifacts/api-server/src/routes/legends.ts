...
import { chargeMintFee } from "../lib/charge-mint";
...
export function toLegend(row: {
  ...
  return {
    name: row.name, position: row.position, pace: row.pace, shooting: row.shooting,
    passing: row.passing, dribbling: row.dribbling, defending: row.defending, physical: row.physical,
    overall: ovr, profileComplete: row.profileComplete,
    allocatedKchip: allocated(row), allocatedKtk: allocated(row),
    mint, perkId: mint?.perkId ?? null, ruleset: mint?.ruleset ?? RULESET,
  };
}
...
router.post("/legends/mint", async (req, res) => {
  try {
    ...
    let feeKtk = 0;
    try {
      feeKtk = await chargeMintFee(identity.privyUserId);
    } catch (err) {
      return res.status(400).json({ error: err instanceof Error ? err.message : "Mint fee failed" });
    }
    ...
    const record: MintRecord = {
      ...
      ruleset: RULESET,
      snapshot: {
        name: legend.name, position: legend.position, overall: ovr,
        stats: {
          pace: legend.pace, shooting: legend.shooting, passing: legend.passing,
          dribbling: legend.dribbling, defending: legend.defending, physical: legend.physical,
        },
        allocatedKtk: allocKtk, avatarSeed: seed,
      },
    };
    ...
    return res.json({
      ...
      feeKtk: 10,
      playableKtk: feeKtk,
      message: existingMint ? `Paid $1 stand-in. Reminted with ${perkLabel(perkId)}.` : `Paid $1 stand-in. Minted with ${perkLabel(perkId)}.`,
      mint: record,
...
router.get("/legends/token/:id", (req, res) => {
  ...
  for (const record of mintStore.values()) {
    if (record.tokenId === id) { found = record; break; }
  }
  ...
  return res.json({
    name: `${name} (OVR ${ovr})`,
    description: "Katika living player card.",
...
router.get("/leaderboard", async (_req, res) => {
  try {
    ...
    return res.json({ leaderboard: legends.map((l, idx) => ({ rank: idx + 1, name: l.name, position: l.position })) });
  } catch {
...
router.put("/legends/me", async (req, res) => {
  try {
    ...
    const stats = {
      pace: clamp(Number(body.pace)), shooting: clamp(Number(body.shooting)), passing: clamp(Number(body.passing)),
      dribbling: clamp(Number(body.dribbling)), defending: clamp(Number(body.defending)), physical: clamp(Number(body.physical)),
    };
    ...
    const volumeRows = await db.select({ volume: sql<number>`coalesce(sum(${gameBetsTable.wager}), 0)` }).from(gameBetsTable).where(eq(gameBetsTable.privyUserId, identity.privyUserId));
    const wagered = Number(volumeRows[0]?.volume ?? 0);
    const rolloverNeed = ktkRolloverNeed();
    const unlocked = wagered >= rolloverNeed;
    const cap = unlocked ? bank : Math.min(bank, KTK_FIRST_CAP);
    if (needed > cap) {
      return res.status(400).json({ error: unlocked ? `Need ${needed} KTK. Bank is ${bank}.` : `First card can lock ${KTK_FIRST_CAP} KTK.` });
    }
    ...
    if (existing[0]) await db.update(legendsTable).set(values).where(eq(legendsTable.privyUserId, identity.privyUserId));
    else await db.insert(legendsTable).values(values);
    ...
    const saved = await db.select().from(legendsTable).where(eq(legendsTable.privyUserId, identity.privyUserId)).limit(1);
    return res.json({ ...toLegend(saved[0], identity.privyUserId), playableKchip: nextPlayable });
  } catch (error) {
    if (error instanceof AuthConfigError) return res.status(503).json({ error: error.message });
    if (error instanceof AuthError) return res.status(401).json({ error: error.message });
